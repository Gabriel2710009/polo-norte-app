...
async def _esperar_y_verificar(bot, user_id, record_id, logs_channel_id, alert_channel_id, alert_role_id):
    ...
    try:
        ...
        if row[1]:
            ...
            log_actions.log_info("✅ Táser devuelto", f"<@{user_id}> devolvió el táser dentro del tiempo.")
            return
        if row[2]:
        ...
        logger.warning("ALERTA: Táser NO devuelto para %s (record=%s)", user_id, record_id)
        log_actions.log_warning(
            "⚠️ Táser NO devuelto",
        ...
        if channel:
            ...
            alert.add_field(name="📋 Estado", value=f"Retiró un táser y no lo devolvió dentro de los {CHECK_WINDOW_MINUTES} minutos posteriores al fichaje de salida.", inline=False)
            await channel.send(content=f"<@&{alert_role_id}>", embed=alert)
            logger.info("Alerta enviada a canal %s", alert_channel_id)
